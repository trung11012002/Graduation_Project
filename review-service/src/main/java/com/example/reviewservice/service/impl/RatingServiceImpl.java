...
public class RatingServiceImpl implements RatingService {
    ...
    public ApiResponse createRating(RatingRequest dto) {
        ...
        Film film =
                filmRepository.findById(dto.getFilmId()).orElseThrow(() -> new AppException(ErrorCode.FILM_NOT_FOUND));
    ...
    public ApiResponse getFilmRatings(Integer filmId) {
        ...
        List<RatingResponse> ratingResponses = ratings.stream()
                .map(rating -> ratingMapper.toRatingResponse(rating))
                .toList();
    ...
    public RatingResponse getRatingInFilmByUser(Integer filmId, Integer userId) {
        Rating rating = ratingRepository.findByFilmIdAndUserId(filmId, userId).orElse(null);
        return (rating != null)
                ? ratingMapper.toRatingResponse(rating)
                : null; // Trả về đối tượng rỗng nếu không có rating
    }
}
